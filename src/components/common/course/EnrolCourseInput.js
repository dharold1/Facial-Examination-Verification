...
export default () => {
  ...
  return (
    <div>
      <p className='alert'>🢃 Enter Course ID for new exam enrolment</p>

      <Form style={{ display: 'flex' }} onFinish={() => enrolCourseCallback()}>
        <Form.Item
          label='Course ID'
          name='courseID'
          rules={[{ required: true, message: 'Please input exam course ID!' }]}
        >
          <Input
            name='courseCode'
            placeholder='Enter exam course ID to enrol'
